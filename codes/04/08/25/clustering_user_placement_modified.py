...
N, radius = 500, 5000  # Users, cell radius (m) - Updated to accommodate zone requirements
...
# -------------------- User Placement --------------------
# Define radial zones with exact minimum user counts
zone_definitions = [
]

# Calculate minimum required users
total_min_users = sum(zone[2] for zone in zone_definitions)
remaining_users = N - total_min_users

print(f"Total users: {N}")
print(f"Minimum required users: {total_min_users}")
print(f"Remaining users for flexible zones: {remaining_users}")

# Use exact counts from zone definitions
zone_user_counts = [zone[2] for zone in zone_definitions]

print("Final user distribution by zone:")
for i, (r_min, r_max, _) in enumerate(zone_definitions):
    print(f"Zone {i+1} ({r_min}-{r_max}m): {zone_user_counts[i]} users")

# Generate user positions
r = np.zeros(N)
theta = np.random.uniform(0, 2*np.pi, N)
user_zones = np.zeros(N, dtype=int)

user_idx = 0
for zone_idx, ((r_min, r_max, _), count) in enumerate(zip(zone_definitions, zone_user_counts)):
    if count > 0:
        # Generate uniform distribution within the annular zone
        r_min_sq = r_min**2
        r_max_sq = r_max**2
        r_zone = np.sqrt(np.random.uniform(r_min_sq, r_max_sq, count))
        
        # Assign to users
        r[user_idx:user_idx + count] = r_zone
        user_zones[user_idx:user_idx + count] = zone_idx
        user_idx += count

# Convert to Cartesian coordinates
x_coords, y_coords = r * np.cos(theta), r * np.sin(theta)

print(f"User placement complete. Placed {user_idx} users total.")
...
#def PL_UMa_LOS(d_2D, d_3D, fc, h_BS, h_UT):
# Breakpoint distance
# h_E = 1.0  # Environment height in meters
#    d_bp = (4 * (h_BS - h_E) * (h_UT - h_E) * fc) / c
#    if d_2D <= d_bp:
#    PL = 28.0 + 22 * np.log10(d_3D) + 20 * np.log10(fc / 1e9)
#    else:
...
for i in range(N):
    if np.random.rand() <= P_LOS_users[i]:
        ...
        PL_dB[i] = max(PL_UMa_LOS(d_3D[i], fc),
                       PL_UMa_NLOS(d_3D[i], fc, h_UTs[i]))
        is_LOS[i] = False
...
# -------------------- Channel Gain --------------------
# Combine path loss, shadowing, and fading
pl_with_shadowing_linear = 10 ** (-(PL_dB + shadowing) / 10)
...
# Now h_values and h_db are ready for clustering
...
def perform_clustering(pairs_indices, name):
    ...
    for u1, u2 in tqdm(pairs_indices, desc=f"Processing {name} pairs"):
        ...
        if sic_satisfied(min(h1, h2), max(h1, h2)):
            ...
            # SIC threshold check value
            sic_margin_dB = 10 * np.log10(max(h1, h2) / min(h1, h2))
            sic_satisfied_flag = sic_margin_dB >= sic_threshold_db
            
            data.append([
                u1, u2, h1, h2, P1, P2, R1, R2, R_sum, "NOMA",
                # User 1 detailed calculations
                d_2D_u1, h_UT_u1, d_3D_u1, user_zones[u1], LOS_status_u1, channel_type_u1, 
                PL_dB_u1, PL_linear_u1, shadowing_dB_u1, fading_u1, prob_LOS_u1,
                # User 2 detailed calculations  
                d_2D_u2, h_UT_u2, d_3D_u2, user_zones[u2], LOS_status_u2, channel_type_u2,
                PL_dB_u2, PL_linear_u2, shadowing_dB_u2, fading_u2, prob_LOS_u2,
                # SIC analysis
    ...
    for u in range(N):
        if not used[u]:
            ...
            data.append([
                ...
                d_2D_u, h_UT_u, d_3D_u, user_zones[u], LOS_status_u, channel_type_u,
                PL_dB_u, PL_linear_u, shadowing_dB_u, fading_u, prob_LOS_u,
                # User 2 (empty for OMA)
                -1, -1, -1, -1, False, "N/A", -1, -1, -1, -1, -1,
    ...
    cols = [
        ...
        # User 1 detailed calculations
        "U1_Distance_2D_m", "U1_Height_m", "U1_Distance_3D_m", "U1_Zone", "U1_LOS_Status", "U1_Channel_Type",
        "U1_PathLoss_dB", "U1_PathLoss_Linear", "U1_Shadowing_dB", "U1_Fading", "U1_LOS_Probability",
        # User 2 detailed calculations
        "U2_Distance_2D_m", "U2_Height_m", "U2_Distance_3D_m", "U2_Zone", "U2_LOS_Status", "U2_Channel_Type", 
        "U2_PathLoss_dB", "U2_PathLoss_Linear", "U2_Shadowing_dB", "U2_Fading", "U2_LOS_Probability",
        # SIC analysis
