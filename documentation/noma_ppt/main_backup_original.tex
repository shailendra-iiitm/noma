...
\definecolor{lightgreen}{RGB}{200,255,200} % Light green for table highlights
...
\begin{frame}{The 5G/6G Challenge}
\begin{columns}
  \begin{column}{0.5\textwidth}
    \textbf{Growing Demands:}
    \begin{itemize}
      \item Exponential device growth
      \item Limited spectrum resources
      \item High data rate requirements
      \item Massive IoT connectivity
    \end{itemize}
  \end{column}
  \begin{column}{0.5\textwidth}
    \textbf{Traditional Limitations:}
    \begin{itemize}
      \item OMA inefficiency
      \item Poor cell-edge performance
    \end{itemize}
  \end{column}
\end{columns}

\vspace{0.3cm}
\begin{alertblock}{Solution: Non-Orthogonal Multiple Access (NOMA)}
Multiple users share same resources through \textbf{power-domain multiplexing}.
\end{alertblock}
\end{frame}
...
\begin{frame}{The User Pairing Challenge}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \textbf{Requirements:}
            \begin{itemize}
                \item $\checkmark$ Sufficient channel gain difference (SIC)
                \item $\checkmark$ Adequate angular separation
                \item $\checkmark$ Maximize sum-rate
                \item $\checkmark$ Ensure fairness
                \item $\checkmark$ Real-time decision
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \textbf{Challenges:}
            \begin{itemize}
                \item $\times$ Combinatorial complexity: $O(N^3)$
                \item $\times$ 500 users = $10^{1134}$ combinations!
                \item $\times$ Real-time constraint (<1 sec)
                \item $\times$ Dynamic channel conditions
                \item $\times$ Multi-objective optimization
            \end{itemize}
        \end{column}
    \end{columns}
...
\begin{frame}{Why Graph Neural Networks?}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \textbf{Advantages:}
            \begin{itemize}
                \item $\checkmark$ Natural graph structure
                \item $\checkmark$ Learn from optimal solutions
                \item $\checkmark$ Fast inference
                \item $\checkmark$ Generalization to new scenarios
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
...
\begin{frame}{Throughput Performance}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \textbf{Key Results:}
            \begin{itemize}
                \item $\checkmark$ \textbf{96.5\%} of optimal throughput
                \item $\checkmark$ \textbf{52.9× speedup} over Blossom
                \item $\checkmark$ \textbf{73\% better} than simple heuristics
                \item $\checkmark$ Real-time inference (<1 sec)
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
...
\begin{frame}{Computational Complexity Comparison}
    \begin{table}
        ...
        \begin{tabular}{lccc}
            ...
            \midrule
            Static & $O(N \log N)$ & $O(N)$ & $\checkmark$ Yes \\
            Balanced & $O(N \log N)$ & $O(N)$ & $\checkmark$ Yes \\
            Bipartite & $O(N^3)$ & $O(N^2)$ & $\times$ No (35s) \\
            Blossom & $O(N^3)$ & $O(N^2)$ & $\times$ No (45s) \\
            \midrule
            \rowcolor{lightgreen}
            \textbf{NOMA-GNN} & $O(N \log N)$ & $O(N)$ & $\checkmark$ \textbf{Yes (0.85s)} \\
            \bottomrule
        \end{tabular}
...
\begin{frame}{Summary}
    ...
    \textbf{Achievements:}
    \begin{itemize}
        \item $\checkmark$ \textbf{96.5\%} of optimal throughput (70.74 vs 73.30 Gbps)
        \item $\checkmark$ \textbf{52.9× speedup} (846 ms vs 44.7 s)
        \item $\checkmark$ \textbf{Real-time inference} for 500 users
        \item $\checkmark$ \textbf{Lightweight model} (166K params, 650 KB)
        \item $\checkmark$ Practical deployment for dense 5G/6G networks
    \end{itemize}
\end{frame}
