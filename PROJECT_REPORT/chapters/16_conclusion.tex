...
\begin{itemize}
    \item Compared against Static, Balanced, Blossom, Bipartite PF methods
    \item Evaluated on throughput, fairness, runtime, generalization
    \item Demonstrated 95.8\% optimal throughput with 20× speedup
    \item Achieved near-optimal fairness (Jain's index 0.903)
\end{itemize}
...
\begin{enumerate}
    \item NOMA-GNN achieves \textbf{95.8\% of optimal throughput}, outperforming all polynomial-time baselines
    \item Classification AUC of \textbf{92.5\%} indicates excellent pairing discrimination
    \item Regression MAE of \textbf{0.031 bps/Hz} for sum-rate prediction demonstrates accurate rate estimation
    \item Jain's fairness index of \textbf{0.903} ensures equitable user service
\end{enumerate}

\subsection{Efficiency}
\begin{enumerate}
    \item Runtime of \textbf{1.1 ms} for 10 users enables real-time scheduling
    \item \textbf{20× speedup} over Blossom matching ($O(N^3)$)
    \item Near-linear scalability ($O(N \log N)$) up to 16 users
    \item Inference latency $< 1$ TTI (1 ms) for practical deployment
\end{enumerate}

\subsection{Generalization}
\begin{enumerate}
    \item Maintains \textbf{$>92\%$ throughput} on unseen user densities ($N=14, 16$)
    \item Robust to channel variations (94.3\% under high shadowing)
    \item Transfers to alternative cell geometries (93.7-94.1\%)
